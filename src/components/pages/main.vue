...
export default {
  ...
  computed: {
    formattedMembers() {
      ...
      const membersArray = this.totalMembers[0]
        .toString()
        .split('')
        .map(Number);

      while (membersArray.length < 5) {
  ...
  methods: {
    ...
    async fetchTotalMembers() {
      try {
        ...
        this.totalMembers = [response.data.totalMembers.toString()];
        console.log('Total Members:', this.totalMembers.join(' '));
      } catch (error) {
